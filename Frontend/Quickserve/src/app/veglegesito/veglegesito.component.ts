...
checkCoupon() {
  ...
  this.rendelesservice.checkCoupon(this.couponCode).subscribe({
    next: (response) => {
      ...
      this.discountId = response.discountId;
...
submitOrder() {
 ...
 const megrendelesId = this.generateShortOrderId();
  ...
  const DiscountId= this.getDiscountId();;

  this.rendelesservice.sendOrder({ items: orderItems, totalDiscountedPrice,Discount,DiscountId }).subscribe({
    next: (response) => {
