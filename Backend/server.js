...
app.post("/register", async (req, res) => {
  ...
  const sql = "INSERT INTO Vásárlók (Nev, Telefonszam, Lakcim,Email,Jelszo,Osztaly) VALUES (?, ?, ?,?,?,?)";

  db.query(sql, [Nev, Telefonszam, Lakcim,Email,hashedPassword,Osztaly], (err, result) => {
    if (err) return res.status(500).json({ error: "Hiba a regisztrációnál" });
    res.json({ message: "Sikeres regisztráció!" });
    console.log("regisztracio tortent");
  });
});
...
app.get("/admin/teljesosszeg/:rendelesid",(req,res)=>{
  const {rendelesid} = req.params;
  const query="SELECT megrendelések.Megrendeles_ID,SUM(termékek.egysegar*megrendelések.mennyiseg) As Osszeg,megrendelések.Kedvezmenyes_osszeg,megrendelések.Kedvezmeny,Kuponok.Kuponkod FROM megrendelések INNER JOIN termékek ON megrendelések.cikkszam=termékek.cikkszam INNER JOIN Kuponok ON megrendelések.Kupon_ID=Kuponok.ID WHERE megrendelések.Megrendeles_ID=?"
  const token=req.headers.authorization;
...
app.post("/checkcoupon", (req, res) => {
  ...
  const sql = "SELECT Arengedmeny,Id FROM kuponok WHERE Kuponkod = ?";
  db.query(sql, [couponCode], (err, results) => {
    ...
    res.json({ discount, discountId }); // Visszaadjuk a kedvezményt
  });
});
...
app.post("/sendorder", (req, res) => {
  const {items, totalDiscountedPrice,Discount,DiscountId} = req.body;
  const token = req.headers.authorization;
  const sql1 = "INSERT INTO megrendelések(Megrendeles_ID, Vasarlo_ID, Cikkszam,Datum, Mennyiseg, Szunet,Fizetesi_mod,Kedvezmenyes_osszeg,Kedvezmeny,Kupon_ID) VALUES ?";
  const sql2 = "INSERT INTO rendelés_állapot(Megrendeles_ID, Statusz) VALUES (?, 'Rendelés elküldve')";
  if (!token) return res.status(401).json({ message: "Nincs bejelentkezve" });
  jwt.verify(token, "secret", (err, decoded) => {
    if (err) return res.status(403).json({ message: "Érvénytelen token" });
    const values = items.map(t => [t.Megrendeles_ID, decoded.Vasarlo_ID, t.Cikkszam,t.Datum, t.Mennyiseg, t.Szunet,t.Fizetesi_mod,totalDiscountedPrice,Discount,DiscountId]);
